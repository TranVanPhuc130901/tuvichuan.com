...
public partial class Areas_Admin_Control_Service_Item_Index : System.Web.UI.UserControl
{
    ...
    private readonly string _subapp = CodeApplications.ServiceImagesOther;
    protected readonly string Pic = FolderPic.Service;
    private string _p = "1";
    private string _numberShowItem = "10";
    private string _igid = "";
    private string _key = "";
    private string _sortCookiesName = SortKey.SortServiceItems;
    ...
    private void GetList(string order)
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            if (ServiceConfig.ShowMorePictures) s.Append("<a href=\"javascript:NewWindow_('" + LinkAddImageToItem(id) + "','ImageList','800','500','yes','yes');\" title='Thêm hình ảnh' class='btn btn-default'><i class='fa fa-picture-o'></i></a> ");
            s.Append("<a href='" + LinkAdmin.GoAdminOption(CodeApplications.Service, TypePage.UpdateItem, "iiid", id) + "' title='Chỉnh sửa' class='btn btn-default'><i class='fa fa-pencil'></i></a> ");
