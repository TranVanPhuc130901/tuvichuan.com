...
public partial class Areas_Admin_Control_Service_Item_AddEditItem : UserControl
{
    private readonly string _lang = RevosJsc.LanguageControl.Cookie.GetLanguageValueAdmin();
    private readonly string _app = CodeApplications.Service;
    private readonly string _pic = FolderPic.Service;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        UploadImage.Text = "Ảnh đại diện (300x300 px)";
    ...
    {
        return LinkAdmin.GoAdminCategory(CodeApplications.Service, TypePage.Item, ddlCategory.SelectedValue);
    }
    ...
    private void InitialControlsValue(bool isInsert)
    {
        ...
        if (!isInsert)
        {
            ltrTitle.Text = ServiceKeyword.CapNhatBaiViet;
        ...
        {
            ltrTitle.Text = ServiceKeyword.ThemMoiBaiViet;
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        if (_insert)
        {
            ...
            GroupItems.InsertItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image1), "", txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), "0", "0", "", "0", txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString());
            #region Logs
        ...
        else
        {
            ...
            GroupItems.UpdateItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image1), "", txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), "0", "0", "", HdTotalView.Value, txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString(), _iiid, HdOldIgId.Value);
            #region Logs
            var logAuthor = CookieExtension.GetCookies(SecurityExtension.BuildPassword(UsersColumns.VuAccount));
            var logCreateDate = DateTime.Now.ToString();
