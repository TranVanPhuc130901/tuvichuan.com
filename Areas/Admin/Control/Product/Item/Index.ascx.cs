...
public partial class Areas_Admin_Control_Product_Item_Index : System.Web.UI.UserControl
{
    ...
    private string _key = "";
    private string _sortCookiesName = SortKey.SortProductItems;
    ...
    private void GetList(string order)
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s.Append("<div class=\"cot2\">" + ImagesExtension.GetImage(Pic, image, itemTitle, "w90px mr5", true, false, "") + dt.Rows[i][ItemsColumns.ViTitle] + "</div>");
            ...
            s.Append("<a href=\"javascript:NewWindow_('/Areas/Admin/PopUp/Items/AddVariantsToItems.aspx?control=" + Control + "&id=" + id + "','ImageList','800','500','yes','yes');\" title='Thêm biến thể' class='btn btn-default'><i class='fa fa-exchange'></i></a> ");

            //s.Append("<a href=\"javascript:NewWindow_('/Areas/Admin/PopUp/Items/AddNewsForItems.aspx?control=" + Control + "&iiid=" + id + "','ImageList','800','500','yes','yes');\" title='Thêm tin tức liên quan' class='btn btn-default'><i class='fa fa-new
            if (ProductConfig.ShowMorePictures) s.Append("<a href=\"javascript:NewWindow_('/Areas/Admin/PopUp/Items/AddImageToItem.aspx?control=" + Control + "&subapp=" + _subapp + "&pic=" + Pic + "&iiid=" + id + "','ImageList','800','500','yes','yes');\" title='Thêm ảnh' class='btn btn-default'><i class='fa fa-picture-o'></i></a> ");

            //s.Append("<a target='_blank' href=\"/Areas/Admin/PopUp/Items/AddImageToItem2.aspx?control=" + Control + "&subapp=" + _subapp + "2&pic=" + Pic + "&iiid=" + id + "\" title='Thêm ảnh thực tế' class='btn btn-default'><i class='fa fa-picture-o'></i></a> ");

            //s.Append("<a target='_blank' href='/Areas/Admin/PopUp/Items/AddFormalityToItem.aspx?app=Product&iiid=" + id + "&subapp=Formality' title='Thêm hình thức' class='btn btn-default'><i class='fa fa-cog'></i></a> ");
            s.Append("<br/>");
            //s.Append("<a href=\"javascript:ClonePro(" + dt.Rows[i][GroupsColumns.IgId] + "," + id + ",'" + _app + "');\" title=\"Tạo bản sao\" class='btn btn-default'><i class=\"gi gi-more_windows\"></i></a> ");
            ...
            s.Append("<a href='" + LinkAdmin.GoAdminOption(CodeApplications.Product, TypePage.UpdateItem, "iiid", id) + "' title='Chỉnh sửa' class='btn btn-default'><i class='fa fa-pencil'></i></a> ");
