...
public partial class Areas_Admin_Control_Product_Category_AddEditCategory : UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        UploadImage2.Text = "Banner mobile";
    ...
    {
        return LinkAdmin.GoAdminSubControl(CodeApplications.Product, TypePage.Category, ddlCategory.SelectedValue);
    }
    ...
    private void InitialControlsValue(bool isInsert)
    {
        ...
        if (!isInsert)
        {
            ...
            UploadImage2.Load(StringExtension.LayChuoi(dt.Rows[0][GroupsColumns.VgImage].ToString(), "", 3));
            txtLinkBanner.Text = StringExtension.LayChuoi(dt.Rows[0][GroupsColumns.VgImage].ToString(), "", 4);
            ...
            HdParams.Value = dt.Rows[0][GroupsColumns.VgParam].ToString();
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        if (_insert)
        {
            ...
            var image2 = UploadImage2.Save(false, "");
            Groups.Insert(ddlCategory.SelectedValue, _lang, _app, txtOrder.Text, "", "0",  txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image1, image2, txtLinkBanner.Text), HdParams.Value, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status);
            #region Logs
        ...
        else
        {
            ...
            var image2 = UploadImage2.Save(true, "");
            Groups.Update(ddlCategory.SelectedValue, _app, txtOrder.Text, "", HdTotalView.Value, txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image1, image2, txtLinkBanner.Text), HdParams.Value, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, _igid);
            #region Logs
            var logAuthor = CookieExtension.GetCookies(SecurityExtension.BuildPassword(UsersColumns.VuAccount));
            var logCreateDate = DateTime.Now.ToString();
