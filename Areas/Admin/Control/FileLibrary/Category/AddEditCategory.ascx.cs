...
public partial class Areas_Admin_Control_FileLibrary_Category_AddEditCategory : UserControl
{
    ...
    {
        return LinkAdmin.GoAdminSubControl(CodeApplications.FileLibrary, TypePage.Category, ddlCategory.SelectedValue);
    }
    ...
    private void InitialControlsValue(bool isInsert)
    {
        ...
        if (!isInsert)
        {
            ltrTitle.Text = FileLibraryKeyword.CapNhatDanhMuc;
            ...
            txtPriceNew.Text = dt.Rows[0][GroupsColumns.VgParam].ToString();
        ...
        {
            ltrTitle.Text = FileLibraryKeyword.ThemMoiDanhMuc;
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        if (_insert)
        {
            ...
            Groups.Insert(ddlCategory.SelectedValue, _lang, _app, txtOrder.Text, "", "0",  txtTitle.Text, txtDesc.Text, contentDetail, image, txtPriceNew.Text, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status);
            #region Logs
        ...
        else
        {
            ...
            Groups.Update(ddlCategory.SelectedValue, _app, txtOrder.Text, "", HdTotalView.Value, txtTitle.Text, txtDesc.Text, contentDetail, image, txtPriceNew.Text, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, _igid);
            #region Logs
            var logAuthor = CookieExtension.GetCookies(SecurityExtension.BuildPassword(UsersColumns.VuAccount));
            var logCreateDate = DateTime.Now.ToString();
