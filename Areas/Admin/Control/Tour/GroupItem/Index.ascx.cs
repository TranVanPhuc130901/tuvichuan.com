...
public partial class Areas_Admin_Control_Tour_GroupItem_Index : System.Web.UI.UserControl
{
    ...
    private const string App = CodeApplications.TourGroupItem;
    ...
    private string LinkAddFilter(string igid)
    {
        return "/Areas/Admin/PopUp/Groups/AddFilter.aspx?igid=" + igid;
    }
    private string GetCate()
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            var link = dt.Rows[i][GroupsColumns.VgLink].ToString();
            ...
            if (link.Length > 0) s.Append("<a target='_blank' href='" + UrlExtension.WebsiteUrl + dt.Rows[i][GroupsColumns.VgLink].ToString().ToLower() + RewriteExtension.Extensions + @"' title='Xem trang hiển thị' class='btn btn-default'><i class='fa fa-eye'></i></a> ");
            if (link.Length > 0) s.Append("<a href=\"javascript:NewWindow_('" + LinkAddFilter(id) + "','ImageList','900','500','no','no')\" title=\"Thêm thuộc tính lọc\" class='btn btn-default'><i class='fa fa-filter'></i></a> ");
