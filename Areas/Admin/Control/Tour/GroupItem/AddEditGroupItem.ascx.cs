...
public partial class Areas_Admin_Control_Tour_GroupItem_AddEditGroupItem : UserControl
{
    private readonly string _lang = RevosJsc.LanguageControl.Cookie.GetLanguageValueAdmin();
    private readonly string _app = CodeApplications.TourGroupItem;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        UploadImage.Control = CodeApplications.Tour;
        ...
        UploadImage1.Control = CodeApplications.Tour;
    ...
    {
        var listModul = new TourPosition();
    ...
    {
        return LinkAdmin.GoAdminSubControl(CodeApplications.Tour, TypePage.GroupItem);
    }
    ...
    private void InitialControlsValue(bool isInsert)
    {
        ...
        if (!isInsert)
        {
            ltrTitle.Text = TourKeyword.CapNhatNhom;
        ...
        {
            ltrTitle.Text = TourKeyword.ThemMoiNhom;
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        if (_insert)
        {
            ...
            Groups.Insert("0", _lang, _app, txtOrder.Text, DdlPosition.SelectedValue, "0",  txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image1), HdParam.Value, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, "", StringExtension.ReplateTitle(txtUrl.Text), txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status);
            #region Logs
        ...
        else
        {
            ...
            Groups.Update("0", _app, txtOrder.Text, DdlPosition.SelectedValue, HdTotalView.Value, txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image1), HdParam.Value, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, "", StringExtension.ReplateTitle(txtUrl.Text), txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, _igid);
            #region Logs
            var logAuthor = CookieExtension.GetCookies(SecurityExtension.BuildPassword(UsersColumns.VuAccount));
            var logCreateDate = DateTime.Now.ToString();
