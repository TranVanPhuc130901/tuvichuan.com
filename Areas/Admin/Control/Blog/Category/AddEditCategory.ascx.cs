...
public partial class Areas_Admin_Control_Blog_Category_AddEditCategory : UserControl
{
    ...
    {
        return LinkAdmin.GoAdminSubControl(CodeApplications.Blog, TypePage.Category, ddlCategory.SelectedValue);
    }
    ...
    private void InitialControlsValue(bool isInsert)
    {
        ...
        if (!isInsert)
        {
            ltrTitle.Text = BlogKeyword.CapNhatDanhMuc;
        ...
        {
            ltrTitle.Text = BlogKeyword.ThemMoiDanhMuc;
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        else
        {
            ...
            Groups.Update(ddlCategory.SelectedValue, _app, txtOrder.Text, "", HdTotalView.Value, txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image1), "", txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, _igid);
            #region Logs
            var logAuthor = CookieExtension.GetCookies(SecurityExtension.BuildPassword(UsersColumns.VuAccount));
            var logCreateDate = DateTime.Now.ToString();
