...
public partial class Areas_Admin_Control_News_Item_AddEditItem : UserControl
{
    ...
    {
        return LinkAdmin.GoAdminCategory(CodeApplications.News, TypePage.Item, ddlCategory.SelectedValue);
    }
    ...
    {
        var dt = Users.GetData("", UsersColumns.VuAccount, " iuStatus <> 2", "");
        ddlUser.Items.Clear();
        ddlUser.Items.Add(new ListItem("Chọn tác giả", ""));
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ddlUser.Items.Add(new ListItem(dt.Rows[i][UsersColumns.VuAccount].ToString(), dt.Rows[i][UsersColumns.VuAccount].ToString()));
        }
    }
    ...
    private void InitialControlsValue(bool isInsert)
    {
        ...
        if (!isInsert)
        {
            ltrTitle.Text = NewsKeyword.CapNhatBaiViet;
            ...
            txtReadtime.Text = dt.Rows[0][ItemsColumns.FiPriceOld].ToString();
            ...
            ddlUser.SelectedValue = dt.Rows[0][ItemsColumns.ViAuthor].ToString();
        ...
        {
            ltrTitle.Text = NewsKeyword.ThemMoiBaiViet;
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        if (_insert)
        {
            ...
            GroupItems.InsertItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, contentDetail, image, ddlUser.SelectedValue, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), txtReadtime.Text, "0", "", "0", txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString());
            #region Logs
        ...
        else
        {
            ...
            GroupItems.UpdateItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, contentDetail, image, ddlUser.SelectedValue, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), txtReadtime.Text, "0", "", HdTotalView.Value, txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString(), _iiid, HdOldIgId.Value);
            #region Logs
            var logAuthor = CookieExtension.GetCookies(SecurityExtension.BuildPassword(UsersColumns.VuAccount));
            var logCreateDate = DateTime.Now.ToString();
