...
public partial class Areas_Admin_Control_Member_Item_Index : System.Web.UI.UserControl
{
    private readonly string _lang = RevosJsc.LanguageControl.Cookie.GetLanguageValueAdmin();
    protected readonly string Control = CodeApplications.Member;
    protected readonly string Action = TypePage.Item;
    ...
    private void GetList(string order)
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            var itemTitle = dt.Rows[i][MembersColumns.VmAccount].ToString();
            ...
            s.Append("<div class=\"cot2\">" + ImagesExtension.GetImage(Pic, image, itemTitle, "w90px mr5", true, false, "") + itemTitle + "</div>");
            s.Append("<div class=\"cot3 text-center\">" + ((DateTime)dt.Rows[i][MembersColumns.DmDateCreated]).ToString("dd-MM-yyyy HH:mm") + "</div>");
            s.Append("<div class=\"cot4 text-center\">" + ((DateTime)dt.Rows[i][MembersColumns.DmBirthday]).ToString("dd-MM-yyyy") + "</div>");
            ...
            s.Append("<a href='" + LinkAdmin.GoAdminOption(Control, TypePage.UpdateItem, "imid", id) + "' title='Chỉnh sửa' class='btn btn-default'><i class='fa fa-pencil'></i></a> ");
            s.Append("<a href=\"javascript:DeleteMember('"+ Page + "','" + id + "','" + itemTitle + "')\" title=\"Xóa item\" class=\"btn btn-warning\"><i class=\"fa fa-times\"></i></a>");
            s.Append("</div>");
            s.Append("</div>");
        }
