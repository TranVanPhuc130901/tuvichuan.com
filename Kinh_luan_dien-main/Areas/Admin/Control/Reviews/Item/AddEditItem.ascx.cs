...
public partial class Areas_Admin_Control_Reviews_Item_AddEditItem : UserControl
{
    private readonly string _lang = RevosJsc.LanguageControl.Cookie.GetLanguageValueAdmin();
    private readonly string _app = CodeApplications.Reviews;
    private readonly string _pic = FolderPic.Reviews;
    ...
    {
        return LinkAdmin.GoAdminCategory(CodeApplications.Reviews, TypePage.Item, ddlCategory.SelectedValue);
    }
    ...
    private void InitialControlsValue(bool isInsert)
    {
        ...
        if (!isInsert)
        {
            ltrTitle.Text = ReviewsKeyword.CapNhatBaiViet;
            ...
            txtAddress.Text = dt.Rows[0][ItemsColumns.ViParam].ToString();
            #region SEO
        ...
        {
            ltrTitle.Text = ReviewsKeyword.ThemMoiBaiViet;
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        if (_insert)
        {
            ...
            GroupItems.InsertItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, contentDetail, image, "", txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), "", "0", txtAddress.Text, "0", txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString());
            #region Logs
        ...
        else
        {
            ...
            GroupItems.UpdateItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, contentDetail, image, "", txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), "", "0", txtAddress.Text, HdTotalView.Value, txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString(), _iiid, HdOldIgId.Value);
            #region Logs
            var logAuthor = CookieExtension.GetCookies(SecurityExtension.BuildPassword(UsersColumns.VuAccount));
            var logCreateDate = DateTime.Now.ToString();
