...
public partial class Areas_Admin_Control_OurTeam_Item_AddEditItem : UserControl
{
    private readonly string _lang = RevosJsc.LanguageControl.Cookie.GetLanguageValueAdmin();
    private readonly string _app = CodeApplications.OurTeam;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        UploadImage.Text = "Ảnh đại diện (150x150 px)";
        ...
        UploadImage1.Text = "Cờ quốc gia (37x22 px)";
    ...
    {
        return LinkAdmin.GoAdminCategory(CodeApplications.OurTeam, TypePage.Item, ddlCategory.SelectedValue);
    }
    ...
    private void InitialControlsValue(bool isInsert)
    {
        ...
        if (!isInsert)
        {
            ltrTitle.Text = OurTeamKeyword.CapNhatBaiViet;
            ...
            txtPhone.Text = StringExtension.LayChuoi(dt.Rows[0][ItemsColumns.ViParam].ToString(), "", 1);
            txtEmail.Text = StringExtension.LayChuoi(dt.Rows[0][ItemsColumns.ViParam].ToString(), "", 2);
            txtViber.Text = StringExtension.LayChuoi(dt.Rows[0][ItemsColumns.ViParam].ToString(), "", 3);
            txtZalo.Text = StringExtension.LayChuoi(dt.Rows[0][ItemsColumns.ViParam].ToString(), "", 4);
            txtSkype.Text = StringExtension.LayChuoi(dt.Rows[0][ItemsColumns.ViParam].ToString(), "", 5);
            txtMes.Text = StringExtension.LayChuoi(dt.Rows[0][ItemsColumns.ViParam].ToString(), "", 6);
            txtWhatsApp.Text = StringExtension.LayChuoi(dt.Rows[0][ItemsColumns.ViParam].ToString(), "", 7);
            txtFacebook.Text = StringExtension.LayChuoi(dt.Rows[0][ItemsColumns.ViParam].ToString(), "", 8);
            txtAddress.Text = StringExtension.LayChuoi(dt.Rows[0][ItemsColumns.ViParam].ToString(), "", 9);
        ...
        {
            ltrTitle.Text = OurTeamKeyword.ThemMoiBaiViet;
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        var param = StringExtension.GhepChuoi("", txtPhone.Text, txtEmail.Text, txtViber.Text, txtZalo.Text, txtSkype.Text, txtMes.Text, txtWhatsApp.Text, txtFacebook.Text, txtAddress.Text);
        ...
        if (_insert)
        {
            ...
            GroupItems.InsertItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image1), "", txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), "0", "0", param, "0", txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString());
            #region Logs
        ...
        else
        {
            ...
            GroupItems.UpdateItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image1), "", txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), "0", "0", param, HdTotalView.Value, txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString(), _iiid, HdOldIgId.Value);
            #region Logs
            var logAuthor = CookieExtension.GetCookies(SecurityExtension.BuildPassword(UsersColumns.VuAccount));
            var logCreateDate = DateTime.Now.ToString();
