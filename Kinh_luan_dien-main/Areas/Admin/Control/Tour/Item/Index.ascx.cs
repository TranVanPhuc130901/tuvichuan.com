...
public partial class Areas_Admin_Control_Tour_Item_Index : System.Web.UI.UserControl
{
    ...
    private readonly string _subapp = CodeApplications.TourImagesOther;
    protected readonly string Pic = FolderPic.Tour;
    private string _p = "1";
    private string _numberShowItem = "10";
    private string _igid = "";
    private string _key = "";
    private string _sortCookiesName = SortKey.SortTourItems;
    ...
    private string LinkAddImageToItem(string iiid)
    {
        return "/Areas/Admin/PopUp/Items/AddImageToItem.aspx?control=" + Control + "&subapp=" + _subapp + "&pic=" + Pic + "&iiid=" + iiid;
    }
    private string LinkAddItineraryForTour(string iiid)
    {
        return "/Areas/Admin/PopUp/Items/AddItineraryForTour.aspx?control=" + Control + "&subapp=TourItinerary&pic=" + Pic + "&iiid=" + iiid;
    }
    private string LinkAddSubItemsForTour(string id, string app)
    {
        return "/Areas/Admin/PopUp/Items/AddItemsToItem.aspx?control=" + Control + "&app="+ app + "&id=" + id;
    }
    ...
    private void GetList(string order)
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s.Append("<div class=\"cot2\">" + ImagesExtension.GetImage(Pic, StringExtension.LayChuoi(image, "", 1), itemTitle, "w90px mr5", true, false, "") + dt.Rows[i][ItemsColumns.ViTitle] + "</div>");
            ...
            if (TourConfig.ShowMorePictures) s.Append("<a href=\"javascript:NewWindow_('" + LinkAddImageToItem(id) + "','ImageList','800','500','yes','yes');\" title='Thêm hình ảnh' class='btn btn-default'><i class='fa fa-picture-o'></i></a> ");
            s.Append("<a href=\"javascript:NewWindow_('" + LinkAddItineraryForTour(id) + "','ImageList','800','500','yes','yes');\" title='Thêm lịch trình' class='btn btn-default'><i class='fa fa-list-ul'></i></a> ");
            s.Append("<a href=\"javascript:NewWindow_('" + LinkAddSubItemsForTour(id, "Hotel") + "','ImageList','800','500','yes','yes');\" title='Thêm khách sạn' class='btn btn-default'><i class='fa fa-hotel'></i></a> ");
            s.Append("<a href=\"javascript:NewWindow_('" + LinkAddSubItemsForTour(id, "Cruises") + "','ImageList','800','500','yes','yes');\" title='Thêm du thuyền' class='btn btn-default'><i class='fa fa-ship'></i></a> ");
            s.Append("<a href=\"javascript:NewWindow_('" + LinkAddSubItemsForTour(id, "Product") + "','ImageList','800','500','yes','yes');\" title='Thêm phương tiện' class='btn btn-default'><i class='gi gi-cars'></i></a> ");
            s.Append("<a href='" + LinkAdmin.GoAdminOption(CodeApplications.Tour, TypePage.UpdateItem, "iiid", id) + "' title='Chỉnh sửa' class='btn btn-default'><i class='fa fa-pencil'></i></a> ");
