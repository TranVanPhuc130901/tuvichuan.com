...
public partial class Areas_Admin_Control_Video_Item_Index : System.Web.UI.UserControl
{
    private readonly string _lang = RevosJsc.LanguageControl.Cookie.GetLanguageValueAdmin();
    protected readonly string Control = CodeApplications.Video;
    private readonly string _app = CodeApplications.Video;
    private readonly string _subapp = CodeApplications.VideoImagesOther;
    protected readonly string Pic = FolderPic.Video;
    private string _p = "1";
    private string _numberShowItem = "10";
    private string _igid = "";
    private string _key = "";
    private string _sortCookiesName = SortKey.SortVideoItems;
    ...
    private void GetList(string order)
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s.Append("<div class=\"cot2\">" + (image.Length > 0 ? ImagesExtension.GetImage(Pic, image, itemTitle, "w90px mr5", false, false, "") : VideoExtension.GetYouTubeVideoImage(dt.Rows[i][ItemsColumns.ViCode].ToString(), "w90px mr5", "")) + dt.Rows[i][ItemsColumns.ViTitle] + "</div>");
            ...
            if (VideoConfig.ShowMorePictures) s.Append("<a target='_blank' href=\"/Areas/Admin/PopUp/Items/AddImageToItem.aspx?control=" + Control + "&subapp=" + _subapp + "&pic=" + Pic + "&iiid=" + id +"\" title='Thêm bài viết' class='btn btn-default'><i class='fa fa-picture-o'></i></a> ");
            ...
            s.Append("<a href='" + LinkAdmin.GoAdminOption(CodeApplications.Video, TypePage.UpdateItem, "iiid", id) + "' title='Chỉnh sửa' class='btn btn-default'><i class='fa fa-pencil'></i></a> ");
