...
public partial class Areas_Admin_Control_Hotel_Item_Index : System.Web.UI.UserControl
{
    private readonly string _lang = RevosJsc.LanguageControl.Cookie.GetLanguageValueAdmin();
    protected readonly string Control = CodeApplications.Hotel;
    private readonly string _app = CodeApplications.Hotel;
    private readonly string _subapp = CodeApplications.HotelImagesOther;
    protected readonly string Pic = FolderPic.Hotel;
    private string _p = "1";
    private string _numberShowItem = "10";
    private string _igid = "";
    private string _key = "";
    private string _sortCookiesName = SortKey.SortHotelItems;
    ...
    private void GetList(string order)
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            if (HotelConfig.ShowMorePictures) s.Append("<a target='_blank' href=\"/Areas/Admin/PopUp/Items/AddImageToItem.aspx?control=" + Control + "&subapp=" + _subapp + "&pic=" + Pic + "&iiid=" + id +"\" title='Thêm bài viết' class='btn btn-default'><i class='fa fa-picture-o'></i></a> ");
            ...
            s.Append("<a href='" + LinkAdmin.GoAdminOption(CodeApplications.Hotel, TypePage.UpdateItem, "iiid", id) + "' title='Chỉnh sửa' class='btn btn-default'><i class='fa fa-pencil'></i></a> ");
