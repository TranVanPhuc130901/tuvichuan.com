...
public partial class Areas_Admin_Control_FileLibrary_Item_AddEditItem : UserControl
{
    private readonly string _lang = RevosJsc.LanguageControl.Cookie.GetLanguageValueAdmin();
    private readonly string _app = CodeApplications.FileLibrary;
    private readonly string _pic = FolderPic.FileLibrary;
    ...
    {
        return LinkAdmin.GoAdminCategory(CodeApplications.FileLibrary, TypePage.Item, ddlCategory.SelectedValue);
    }
    ...
    private void InitialControlsValue(bool isInsert)
    {
        ...
        if (!isInsert)
        {
            ltrTitle.Text = FileLibraryKeyword.CapNhatBaiViet;
            ...
            UploadFile.Load(StringExtension.LayChuoi(dt.Rows[0][ItemsColumns.ViImage].ToString(), "", 2), dt.Rows[0][ItemsColumns.ViParam].ToString());
        ...
        {
            ltrTitle.Text = FileLibraryKeyword.ThemMoiBaiViet;
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        if (_insert)
        {
            ...
            GroupItems.InsertItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, txt_content.Text, StringExtension.GhepChuoi("", image, file), "", txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), "0", "0", UploadFile.Link, "0", txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString());
            #region Logs
        ...
        else
        {
            ...
            GroupItems.UpdateItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, txt_content.Text, StringExtension.GhepChuoi("", image, file), "", txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), "0", "0", UploadFile.Link, HdTotalView.Value, txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString(), _iiid, HdOldIgId.Value);
            #region Logs
            var logAuthor = CookieExtension.GetCookies(SecurityExtension.BuildPassword(UsersColumns.VuAccount));
            var logCreateDate = DateTime.Now.ToString();
