...
public partial class Areas_Admin_Control_Destination_Category_Index : System.Web.UI.UserControl
{
    private readonly string _lang = RevosJsc.LanguageControl.Cookie.GetLanguageValueAdmin();
    private const string App = CodeApplications.Destination;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        ltrList.Text = GetCategories();
    }
    private string LinkAddItemToGroup(string igId, string genealogy, string subApp)
    {
        return "/Areas/Admin/PopUp/GroupItems/AddItemsForGroup.aspx?control=" + Control + "&subapp="+ subApp + "&igId=" + igId + "&genealogy=" + genealogy;
    }
    private string LinkPickMap(string igId, string parent)
    {
        return "/Areas/Admin/PopUp/Groups/PickMap.aspx?control=" + Control + "&igId=" + igId + "&parent=" + parent;
    }
    private string GetCategories()
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s.Append("<a target='_blank' href='" + UrlExtension.WebsiteUrl + dt.Rows[i][GroupsColumns.VgLink].ToString().ToLower() + RewriteExtension.Extensions + @"' title='Xem trang hiển thị' class='btn btn-default'><i class='fa fa-eye'></i></a> ");
            s.Append("<a href=\"javascript:NewWindow_('" + LinkAddItemToGroup(id, genealogy, "Blog") + "','ImageList','800','500','yes','yes');\" title='Thêm Blog liên quan' class='btn btn-default'><i class='fa fa-exchange'></i></a> ");
            s.Append("<a href=\"javascript:NewWindow_('" + LinkAddItemToGroup(id, genealogy, "Tour") + "','ImageList','800','500','yes','yes');\" title='Thêm Tour liên quan' class='btn btn-default'><i class='fa fa-exchange'></i></a> ");
    ...
    public string GetSubCategories(string parentId)
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s.Append("<a href=\"javascript:NewWindow_('" + LinkPickMap(id, dt.Rows[i][GroupsColumns.IgParentId].ToString()) + "','ImageList','800','500','yes','yes');\" title='Nhập tọa độ' class='btn btn-default'><i class='fa fa-map'></i></a> ");
            s.Append("<a href=\"javascript:NewWindow_('" + LinkAddItemToGroup(id, genealogy, "Tour") + "','ImageList','800','500','yes','yes');\" title='Thêm Tour liên quan' class='btn btn-default'><i class='fa fa-exchange'></i></a> ");
            s.Append("<a href=\"" + LinkAdmin.GoAdminCategory(Control, TypePage.UpdateCategory, id) + "\" title=\"Chỉnh sửa\" class='btn btn-default'><i class='fa fa-pencil'></i></a> ");
            s.Append("<a href=\"javascript:DeleteGroup('" + Control + "','" + Action + "','" + id + "','" + name + "')\"; title='Xóa danh mục' class='btn btn-warning'><i class='fa fa-times'></i></a>");
            s.Append("</div>");
