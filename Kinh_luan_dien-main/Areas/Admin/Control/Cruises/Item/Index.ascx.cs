...
public partial class Areas_Admin_Control_Cruises_Item_Index : System.Web.UI.UserControl
{
    ...
    private readonly string _subapp = CodeApplications.CruisesImagesOther;
    protected readonly string Pic = FolderPic.Cruises;
    private string _p = "1";
    private string _numberShowItem = "10";
    private string _igid = "";
    private string _key = "";
    private string _sortCookiesName = SortKey.SortCruisesItems;
    ...
    private void GetList(string order)
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            if (CruisesConfig.ShowMorePictures) s.Append("<a target='_blank' href=\"/Areas/Admin/PopUp/Items/AddImageToItem.aspx?control=" + Control + "&subapp=" + _subapp + "&pic=" + Pic + "&iiid=" + id +"\" title='Thêm bài viết' class='btn btn-default'><i class='fa fa-picture-o'></i></a> ");
            ...
            s.Append("<a href='" + LinkAdmin.GoAdminOption(CodeApplications.Cruises, TypePage.UpdateItem, "iiid", id) + "' title='Chỉnh sửa' class='btn btn-default'><i class='fa fa-pencil'></i></a> ");
