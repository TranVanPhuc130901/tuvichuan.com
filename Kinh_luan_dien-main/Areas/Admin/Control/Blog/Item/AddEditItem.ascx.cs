...
public partial class Areas_Admin_Control_Blog_Item_AddEditItem : UserControl
{
    private readonly string _lang = RevosJsc.LanguageControl.Cookie.GetLanguageValueAdmin();
    private readonly string _app = CodeApplications.Blog;
    private readonly string _pic = FolderPic.Blog;
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        UploadImage.Text = "Ảnh đại diện (262x175 px)";
    ...
    {
        return LinkAdmin.GoAdminCategory(CodeApplications.Blog, TypePage.Item, ddlCategory.SelectedValue);
    }
    ...
    private void InitialControlsValue(bool isInsert)
    {
        ...
        if (!isInsert)
        {
            ltrTitle.Text = BlogKeyword.CapNhatBaiViet;
            ...
            txtAuthor.Text = dt.Rows[0][ItemsColumns.ViAuthor].ToString();
        ...
        {
            ltrTitle.Text = BlogKeyword.ThemMoiBaiViet;
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        if (_insert)
        {
            ...
            var image2 = UploadImage1.Save(false, contentDetail);
            GroupItems.InsertItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image2), txtAuthor.Text, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), "0", "0", "", "0", txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString());
            #region Logs
        ...
        else
        {
            ...
            var image2 = UploadImage1.Save(true, contentDetail);
            GroupItems.UpdateItemGroupItem(_lang, _app, "", txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image2), txtAuthor.Text, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, txtTag.Text, StringExtension.ReplateTitle(txtUrl.Text), "0", "0", "", HdTotalView.Value, txtOrder.Text, txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, ddlCategory.SelectedValue, DateTime.Now.ToString(), DateTime.Now.ToString(), _iiid, HdOldIgId.Value);
            #region Logs
            var logAuthor = CookieExtension.GetCookies(SecurityExtension.BuildPassword(UsersColumns.VuAccount));
            var logCreateDate = DateTime.Now.ToString();
