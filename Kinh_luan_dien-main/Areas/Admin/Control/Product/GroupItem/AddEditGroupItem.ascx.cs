...
public partial class Areas_Admin_Control_Product_GroupItem_AddEditGroupItem : UserControl
{
    private readonly string _lang = RevosJsc.LanguageControl.Cookie.GetLanguageValueAdmin();
    private readonly string _app = CodeApplications.ProductGroupItem;
    ...
    {
        var listModul = new ProductPosition();
    ...
    {
        return LinkAdmin.GoAdminSubControl(CodeApplications.Product, TypePage.GroupItem);
    }
    ...
    private void InitialControlsValue(bool isInsert)
    {
        ...
        if (!isInsert)
        {
            ltrTitle.Text = ProductKeyword.CapNhatNhom;
            ...
            txtLink.Text = dt.Rows[0][GroupsColumns.VgParam].ToString();
        ...
        {
            ltrTitle.Text = ProductKeyword.ThemMoiNhom;
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        if (_insert)
        {
            ...
            Groups.Insert("0", _lang, _app, txtOrder.Text, DdlPosition.SelectedValue, "0",  txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image1), txtLink.Text, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, "", StringExtension.ReplateTitle(txtUrl.Text), txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status);
            #region Logs
        ...
        else
        {
            ...
            Groups.Update("0", _app, txtOrder.Text, DdlPosition.SelectedValue, HdTotalView.Value, txtTitle.Text, txtDesc.Text, contentDetail, StringExtension.GhepChuoi("", image, image1), txtLink.Text, txtMetaTitle.Text, txtMetaKeyword.Text, txtMetaDescription.Text, "", StringExtension.ReplateTitle(txtUrl.Text), txtDate.Text.Replace("T", " "), DateTime.Now.ToString(), status, _igid);
            #region Logs
            var logAuthor = CookieExtension.GetCookies(SecurityExtension.BuildPassword(UsersColumns.VuAccount));
            var logCreateDate = DateTime.Now.ToString();
