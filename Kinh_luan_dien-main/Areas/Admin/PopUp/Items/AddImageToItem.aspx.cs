...
public partial class Areas_Admin_PopUp_Items_AddImageToItem : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        btnCancel.Attributes["href"] = "/Areas/Admin/PopUp/Items/AddImageToItem.aspx?control=" + _control + "&subapp=" + _subApp + "&pic=" + _pic + "&iiid=" + _iiId;
        ltrNote.Text = "<div class='text-warning'>Lưu ý: Có thể chọn nhiều ảnh cùng 1 lúc</div>";
        ltrTitle.Text = "Thêm ảnh";
        if (IsPostBack) return;
        GetImageConfig();
        pnHanCheKichThuoc.Visible = false;
        pnTaoAnhNho.Visible = false;
        if (ProductConfig.ShowColor) GetListColor();
        GetItemsInfo();
    ...
    private void GetListColor()
    {
        var condition = DataExtension.AndConditon(
            GroupsTSql.GetByApp(RevosJsc.ProductControl.CodeApplications.ProductColors),
            GroupsTSql.GetByStatus("1")
        );
        var dt = Groups.GetData("", "*", condition, GroupsColumns.IgSortOrder);
        ddlColor.Items.Add(new ListItem("Chọn màu sắc", ""));
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ddlColor.Items.Add(new ListItem(dt.Rows[i][GroupsColumns.VgName].ToString(), dt.Rows[i][GroupsColumns.IgId].ToString()));
        }
    }
    ...
    {
        cbHanCheKichThuoc.Checked = SettingsExtension.GetSettingKey(_control + "HanCheKichThuocAnhKhac", _lang).Equals("1");
        tbHanCheW.Text = SettingsExtension.GetSettingKey(_control + "HanCheKichThuocAnhKhac_MaxWidth", _lang);
        tbHanCheH.Text = SettingsExtension.GetSettingKey(_control + "HanCheKichThuocAnhKhac_MaxHeight", _lang);

        cbTaoAnhNho.Checked = SettingsExtension.GetSettingKey(_control + "TaoAnhNhoChoAnhKhac", _lang).Equals("1");
        tbAnhNhoW.Text = SettingsExtension.GetSettingKey(_control + "TaoAnhNhoChoAnhKhac_MaxWidth", _lang);
        tbAnhNhoH.Text = SettingsExtension.GetSettingKey(_control + "TaoAnhNhoChoAnhKhac_MaxHeight", _lang);
    }
    ...
    private void GetSubItems()
    {
        ...
        txtPriceNew.Text = dt.Rows[0][SubitemsColumns.FsiPriceNew].ToString();
        ltrPriceNew.Text = NumberExtension.FormatNumber(dt.Rows[0][SubitemsColumns.FsiPriceNew].ToString());
        ddlColor.SelectedValue = dt.Rows[0][SubitemsColumns.VsiParam].ToString();
        txtEmbed.Text = dt.Rows[0][SubitemsColumns.VsiContent].ToString();
        ...
        cbPk.Checked = dt.Rows[0][SubitemsColumns.VsiDescription].ToString().Equals("1");
    }
    private void GetList()
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s.Append("<a href='/Areas/Admin/PopUp/Items/AddImageToItem.aspx?control=" + _control + "&subapp=" + _subApp + "&pic=" + _pic + "&iiid=" + _iiId + "&isiid=" + id + @"' class='btn btn-default' title='Sửa'><i class='fa fa-pencil'></i></a> ");
...
";
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        if (cbSaveConfig.Checked)
        {
            ...
            SettingsExtension.SetOtherSettingKey(_control + "HanCheKichThuocAnhKhac", cbHanCheKichThuoc.Checked ? "1" : "0", _lang);
            SettingsExtension.SetOtherSettingKey(_control + "HanCheKichThuocAnhKhac_MaxWidth", tbHanCheW.Text, _lang);
            SettingsExtension.SetOtherSettingKey(_control + "HanCheKichThuocAnhKhac_MaxHeight", tbHanCheH.Text, _lang);
            #endregion

            #region Tạo ảnh nhỏ cho ảnh đại diện
            SettingsExtension.SetOtherSettingKey(_control + "TaoAnhNhoChoAnhKhac", cbTaoAnhNho.Checked ? "1" : "0", _lang);
            SettingsExtension.SetOtherSettingKey(_control + "TaoAnhNhoChoAnhKhac_MaxWidth", tbAnhNhoW.Text, _lang);
            SettingsExtension.SetOtherSettingKey(_control + "TaoAnhNhoChoAnhKhac_MaxHeight", tbAnhNhoH.Text, _lang);
            #endregion
        }
        ...
        for (var i = 0; i < listImage.Count; i++)
        {
            ...
            var pk = "0";
            if (cbPk.Checked) pk = "1";

            if (_edit)
            {
                if (vImg == "") vImg = hdImage.Value;
                else ImagesExtension.DeleteImageWhenDeleteItem(_pic, hdImage.Value);
                Subitems.Update(_iiId, _lang, _subApp, txtTitle.Text, pk, txtEmbed.Text, vImg, ddlColor.SelectedValue, txtPriceOld.Text, txtPriceNew.Text, txtOrder.Text, status, DateTime.Now.ToString(), DateTime.Now.ToString(), _isiId);
            }
            else
            {
                Subitems.Insert(_iiId, _lang, _subApp, txtTitle.Text + (i > 0 ? " " + i : ""), pk, txtEmbed.Text, vImg, ddlColor.SelectedValue, txtPriceOld.Text, txtPriceNew.Text, (int.Parse(txtOrder.Text) + i).ToString(), status, DateTime.Now.ToString(), DateTime.Now.ToString());
            }
            #endregion
        ...
        {
            Response.Redirect("/Areas/Admin/PopUp/Items/AddImageToItem.aspx?control=" + _control + "&subapp=" + _subApp + "&pic=" + _pic + "&iiid=" + _iiId);
        }
    }
}