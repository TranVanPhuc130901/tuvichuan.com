...
public partial class Areas_Admin_PopUp_Items_AddItemsForItems : Page
{
    ...
    {
        var dt = Groups.GetAllGroups(" * ", DataExtension.AndConditon(GroupsTSql.GetByApp(_control), GroupsTSql.GetByLang(_lang), "igStatus <> '2'"), "");
    ...
    private void GetProductGroups(string igidInDll)
    {
        ...
        {
            iidInListAdded = dtitem.Rows[0][ItemsColumns.ViAuthor].ToString();
        ...
        var conditionItem = DataExtension.AndConditon(
            ...
            GroupsTSql.GetByApp(_control),
            ItemsTSql.GetByApp(_control)
            );
