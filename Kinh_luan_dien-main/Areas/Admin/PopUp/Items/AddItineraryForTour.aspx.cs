...
public partial class Areas_Admin_PopUp_Items_AddItineraryForTour : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        btnCancel.Attributes["href"] = "/Areas/Admin/PopUp/Items/AddItineraryForTour.aspx?control=" + _control + "&subapp=" + _subApp + "&pic=" + _pic + "&iiid=" + _iiId;
        ltrTitle.Text = "Thêm lịch trình";
    ...
    private void GetSubItems()
    {
        ...
        txtDate.Text = dt.Rows[0][SubitemsColumns.VsiDescription].ToString();
        txt_content.Text = dt.Rows[0][SubitemsColumns.VsiContent].ToString();
    ...
    private void GetList()
    {
        ...
        for (var i = 0; i < dt.Rows.Count; i++)
        {
            ...
            s.Append("<a href='/Areas/Admin/PopUp/Items/AddItineraryForTour.aspx?control=" + _control + "&subapp=" + _subApp + "&pic=" + _pic + "&iiid=" + _iiId + "&isiid=" + id + @"' class='btn btn-default' title='Sửa'><i class='fa fa-pencil'></i></a> ");
        ...
        var split = PagingCollection.SpilitPagesNoRewrite(Convert.ToInt32(dtPager.Rows[0]["TotalRows"]), top, Convert.ToInt32(_p), "/Areas/Admin/PopUp/Items/AddItineraryForTour.aspx?control=" + _control + "&subapp=" + _subApp + "&pic=" + _pic + "&iiid=" + _iiId, "active", "normal", "first", "last", "preview", "next");
        ltrPaging.Text = @"
...
";
    ...
    protected void btSubmit_OnClick(object sender, EventArgs e)
    {
        ...
        if (_edit)
        {
            Subitems.Update(_iiId, _lang, _subApp, txtTitle.Text, txtDate.Text, txt_content.Text, "", "", "0", "0", txtOrder.Text, status, DateTime.Now.ToString(), DateTime.Now.ToString(), _isiId);
        }
        else
        {
            Subitems.Insert(_iiId, _lang, _subApp, txtTitle.Text, txtDate.Text, txt_content.Text, "", "", "0", "0", txtOrder.Text, status, DateTime.Now.ToString(), DateTime.Now.ToString());
        }
        #endregion
        ...
        {
            Response.Redirect("/Areas/Admin/PopUp/Items/AddItineraryForTour.aspx?control=" + _control + "&subapp=" + _subApp + "&pic=" + _pic + "&iiid=" + _iiId);
        }
    }
}